...
function summary(obj) {
    ...
    planName.innerHTML = '${obj.plan.innerText} (${obj.kind ? "yearyly" : "monthly"})';
}
function validateForm() {
    ...
    for (let i = 0; i  < formInputs.length; i++) {
        if(!formInputs[i].value) {
            ...
            findLabel(formInputs[i]).nextElentSibling.style.display = "flex";
        } else {
            valid = true;
            formInputs[i].classList.remove("err");
            findLabel(formInputs[i]).nextElentSibling.style.display = "none"
        }
    }
    return valid;
}
...
function showAddon(ad, val) {
    ...
    if (ad && val) {
        ...
        document.querySelectorAll(".addons").appenCHild(clone);
    } else {
